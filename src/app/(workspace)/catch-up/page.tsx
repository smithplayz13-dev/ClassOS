...
function impactTier(
  task: { dueDate: string; importance: number },
  today: string,
): "High" | "Medium" | "Low" {
    ...
    <>
      ...
      <div className="absence-list">
        {student.absences.map((absence) => {
          ...
          const recoveryDays = remaining.length
            ? Math.ceil(remainingMinutes / Math.max(1, student.dailyStudyLimit))
            : 0;
          const recoveryDate = recoveryDays
            ? addDays(today, recoveryDays - 1)
            : null;
          const scheduledSubjects = student.subjects.flatMap((subj) =>
            subj.timetable
              .filter((e) => e.dayOfWeek === weekday(absence.date))
              .map((e) => ({ ...e, subject: subj })),
          );
          ...
          return (
            <section className="absence-section" key={absence.id}>
              ...
              >
                ...
                >
                  <Clock3 size={12} /> {scheduledSubjects.length}{" "}
                  {scheduledSubjects.length === 1 ? "class" : "classes"}{" "}
                  scheduled
                </span>
                ...
                {scheduledSubjects.length === 0 && (
                  <span className="muted">
                    No classes scheduled — correction not needed
                  </span>
                )}
              </div>
                ...
                >
                  ...
                  <div className="recovery-impact-grid">
                    <div>
                      ...
                      {high.length} {high.length === 1 ? "task" : "tasks"} —
                      blocking upcoming lesson/test or urgent deadline
                    </div>
                    <div>
                      <strong>Medium</strong> {medium.length} — important but
                      not immediately blocking
                    </div>
                    <div>
                      <strong>Low</strong> {low.length} — safe to defer
                    </div>
                  </div>
              ...
              {tasks.length > 0 ? (
                ...
                <EmptyState
                  ...
                  description="No missed work has been attached to this day. Add lesson notes below to extract it."
                />
              )}
