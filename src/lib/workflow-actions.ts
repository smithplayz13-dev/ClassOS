...
export async function resetDemo(): Promise<ActionState> {
  try {
    ...
    await db.$transaction(async (tx) => {
      ...
      for (const subject of subjects)
        await tx.subject.create({
          data: { ...subject, studentId: DEMO_STUDENT_ID },
        });
      ...
      const tasks = [
        ...
        },
        {
          ...
          description:
            "Review supply and demand shifts from the missed lesson.",
          type: "catch_up" as const,
          offset: 2,
          estimatedMinutes: 25,
          importance: 4,
        ...
        await tx.academicTask.create({
          data: {
            ...
            priority: priorityScore(
              { ...task, dueDate } as unknown as Parameters<
    ...
    return {
      success: true,
      message: "Demo workspace reset. Golden path is ready to replay.",
    };
  } catch (error) {
    return errorState(error);
  }
